/*
Program : Implementation of singly linked list and its functions
...
public:
    void in_start(int);   //insertion at the beginning
    void in_last(int);    //insertion at the end
    void del_start();     //deletion from the start
    void del_last();      //deletion from the end
    void search(int);     //searching an element in the linked list
    void in_pos(int,int);  //insertion at the given position
    void del_pos(int);     //deletion from the given position
    void srt();            //Sorting the list
    void rvrs();          //reversing the given list
    int f_length();       //returns the length of the list
    void find_mid();      //finding the mid element of the list
    void display();       //traversal of the list
    l_list();
};
l_list::l_list()
{
...
void l_list::del_last()// as we do not have the address of last-1 element, we have to
{                      //traverse the whole list from the starting
    if(start==last)
    {
        delete start;
    ...
    node* temp=start;
    node* prev;             //to store the position of second last element.
    while(temp->next!=NULL) //here we do not want to enter into the last element (to process previous).
    {
        prev=temp;         //
        temp=temp->next;
    }
